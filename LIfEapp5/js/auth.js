...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (togglePasswordBtn) {
        ...
        togglePasswordBtn.addEventListener('click', () => {
            ...
            togglePasswordBtn.innerHTML = isHidden ? eyeOff : eyeOpen;
        });
    }

    // "SAIBA MAIS" button on left brand panel
    const loginSaibaMaisBtn = document.getElementById('loginSaibaMaisBtn');
    if (loginSaibaMaisBtn) {
        loginSaibaMaisBtn.addEventListener('click', (e) => {
            e.preventDefault();
            hideLoginModal();
    ...
    if (loginForgotBtn) {
        loginForgotBtn.addEventListener('click', (e) => {
            e.preventDefault();
            const msg = (typeof i18n !== 'undefined' && i18n.t) ? i18n.t('login.forgotMsg') : 'Funcionalidade em desenvolvimento. Por favor, contate o administrador.';
            if (typeof showAlert === 'function') showAlert('info', msg);
            else alert(msg);
        });
    }
